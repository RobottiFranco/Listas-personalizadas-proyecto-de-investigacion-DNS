...
def obtener_tipo_de_bloqueo(mesaurment_uid: str, nombre_salida: str, archivo_salida: str):
    ...
    url.armar_consulta_datos_bloqueo(mesaurment_uid)
    
    datos = ClienteAPI(url, 3)
    datos = datos.realizar_solicitud_obtencion()

    # Obtener los valores relevantes de DNS
    ...
    dns_requests = [
        req.get("failure", "No failure") for req in test_keys.get("requests", [])
    ]
    # Concatenar las fallas de las solicitudes DNS
